...
function logMeIn(user) {
    ...
    return fetch('https://testapi.patientio.com/Token', config)
        ...
        return response.json().then(function (user) { return ({ user: user, response: response }); });
    })
        .then(function (_a) {
        var user = _a.user, response = _a.response;
        if (!response.ok) {
...
describe('GetSponsorUserAsync', function () {
    var user = new User('https://testapi.patientio.com');
